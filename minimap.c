...
SDL_Rect MAJMinimap (SDL_Rect Posjoueur,int redemensionement,SDL_Rect camera, int minimapWidth, int minimapHeight)////////pos absoluee
{
	SDL_Rect pos_joueur;
    ...
    // Ensure the red dot stays within the minimap boundaries
    if (pos_joueur.x < 0)
        pos_joueur.x = 0;
...
void afficherminimap(minimap m, SDL_Surface *screen, SDL_Rect playerPosition)
{
	SDL_BlitSurface(m.img_map,NULL,screen,&m.pos_map);
        // Calculate relative position of the player within the minimap
        ...
        m.pos_joueur.x = playerPosition.x;  // Update red dot position based on player's position
