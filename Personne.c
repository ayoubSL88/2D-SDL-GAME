...
void initPerso2(Personne *p)
{
    ...
    p->up = 0;
    p->ground = 200;
    p->position.x = 500; // Adjusted x position for player 2
    p->position.y = p->ground;
    p->position.w = 115;
    p->position.h = 140;
    p->vitesse = 0;
...
void deplacerPerso2(Personne *p, int dt)
{
    ...
    if (p->direction == 0)
    {
        if (p->position.x < 1780) // Adjusted maximum x position for player 2
            p->position.x = p->position.x + p->dx;
    }
    else if (p->direction == 1)
    {
        if (p->position.x > 10) // Adjusted minimum x position for player 2
            p->position.x = p->position.x - p->dx;
    }
